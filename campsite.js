...
// Simulate the equivalent of $_GET in PHP
const searchParam = new URLSearchParams(window.location.search).get('search');
const searchTerm = searchParam ? decodeURIComponent(searchParam) : '';

// Parse JSON data
const campsiteData = JSON.parse(jsonData);

// Filter campsites based on the search term
const filteredCampsites = campsiteData.campsite_en.filter(campsite => {
    return searchTerm === '' || campsite.name.toLowerCase().includes(searchTerm.toLowerCase());
});

// Limit the number of results to 10
const limitedCampsites = filteredCampsites.slice(0, 10);

const campsiteIDs = limitedCampsites.map(campsite => campsite.id);
const imageLinks = limitedCampsites.map(campsite => campsite.img1);
const imageNames = limitedCampsites.map(campsite => campsite.name);

// Output the JSON response
console.log(JSON.stringify({ ids: campsiteIDs, links: imageLinks, names: imageNames }));
